...
<a href="https://shigatsuel.github.io/javascript30-challenge/Day-24(Sticky-Nav)/index.html" target="_blank">Day 24 - Sticky Nav</a>
...
스크롤을 할 시 어느 이상이 되면 Navigation이 고정되는 챌린지이다.<br>
코드는 그렇게 어렵지 않았고 스크롤Y가 내비게이션 노드의 offsetTop보다 커지면 position: fixed를 추가해주는 것 뿐이다.<br>
단지, 알아두어야 할 것이 있는데 포지션을 고정시키게 되면 레이아웃 자체가 바뀌어서 다른 모든 것들의 위치가 바뀔 수 있는데<br>
이것을 자바스크립트에서 패딩을 추가해줌으로써 간단히 해결할 수 있었다.<br>

요즘에는 이렇게 자바스크립트로 하지 않아도 css position 중에서 sticky라는 것이 생겼기 때문에 완전히 동일한 역할을 하는 것으로 알고있다.<br>
자바스크립트로 해도 어렵지 않아서 어떤 것을 사용할지는 사용자의 마음에 따라 달렸다.<br>

마지막으로 저번에 했던 slide image를 복습할 겸 추가해보았는데 저번과 똑같이 되지는 않았다.<br>
패딩을 추가해서 그런것이 아닌가 싶기도 하다.<br>
